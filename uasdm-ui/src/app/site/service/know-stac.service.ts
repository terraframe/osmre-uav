...
export class KnowStacService {
	...
	constructor(private http: HttpClient, private eventService: EventService, private config: ConfigurationService) { }
	...
	item(href: string): Promise<StacItem> {
		...
		return firstValueFrom(this.http.get<StacItem>(href));
	}
	...
	properties(): Promise<StacProperty[]> {
		...
		// return firstValueFrom(this.http.get<StacProperty[]>(this.config.getKnowStacURL() + '/api/stac-property/get-all', { params: params }));
	}
