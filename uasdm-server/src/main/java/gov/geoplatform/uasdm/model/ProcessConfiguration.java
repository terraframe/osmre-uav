...
public interface ProcessConfiguration
{
  ...
  public static ProcessConfiguration parse(UploadInfo uploadInfo)
  {
    ...
    if (!StringUtils.isEmpty(metadata.get(TYPE)))
    {
      ProcessType type = ProcessType.valueOf(metadata.get(TYPE));

      if (type.equals(ProcessType.LIDAR))
      {
        return LidarProcessConfiguration.parse(uploadInfo);
      }

      return ODMProcessConfiguration.parse(uploadInfo);
    }

    throw new UnsupportedOperationException("TYPE parameter missing. Params: " + serialize(metadata));
  }
