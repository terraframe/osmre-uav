...
public class UserInvite extends UserInviteBase
{
  ...
  private static final int expireTime = AppProperties.getInviteUserTokenExpireTime(); // in hours
   ...
   * MdMethod
   * 
   * Initiates a user invite request. If the user already has one in progress, it will be invalidated and a new one will be issued. If the
   ...
   * MdMethod
   * 
   * Completes the user invite request by verifying the token is valid, creating the requested user, and then invalidating the request. 
   * 
   * @param token
   */
  @Authenticate
